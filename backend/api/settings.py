...
from models.database import get_db, RiskRule, EmailConfig, AppConfig, AutoTradeConfig
...
class RiskRuleCreate(BaseModel):
    ...
    rule_type: str          # loss_pct / max_position_usd / max_exposure_usd / min_rate_diff / max_leverage
    threshold: float
    action: str             # close_position / alert_only
...
def update_app_config(body: AppConfigUpdate, db: Session = Depends(get_db)):
    ...
    db.commit()
    reschedule_jobs(
        data_interval=body.data_refresh_interval,
        risk_interval=body.risk_check_interval,
    )
    return {"success": True}
