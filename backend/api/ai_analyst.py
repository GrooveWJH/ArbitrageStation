"""
AI 分析师 API — 挂载在 arbitrage-tool 后端 (port 8000)
读取 ai-trader/logs/ 下的分析 JSON，将参数调整写回 arbitrage.db。
"""
import json
import sqlite3
...
ARBITRAGE_DB  = Path(__file__).resolve().parents[1] / "data" / "arbitrage.db"

ALLOWED_FIELDS = {
    "min_annualized_pct", "entry_minutes_before_funding",
    "max_entry_spread_pct", "spread_gain_exit_threshold_pct",
    "pre_settle_exit_threshold_pct", "switch_min_improvement_pct",
    "max_hold_cycles", "fixed_size_usd", "max_position_usd",
    "spread_entry_z", "spread_exit_z", "spread_stop_z_delta",
    "spread_position_pct", "spread_cooldown_mins",
    "min_rate_diff_pct", "funding_max_positions",
    "min_cross_volume_usd", "min_spot_volume_usd",
}
...
def apply_adjustments(req: ApplyRequest):
    ...
    if req.filename:
        ...
        _update_file(req.filename, {
            "applied_fields": list(applied),
            "applied": (applied | rejected) >= all_keys,
        })
